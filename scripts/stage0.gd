...
@onready var camera = $Cinematics/Camera2D  # Assurez-vous que ce chemin est correct pour votre Camera2D
@onready var animation_player = $Cinematics/AnimationPlayer  # Assurez-vous que ce chemin est correct pour votre AnimationPlayer
@onready var player = $player  # Assurez-vous que ce chemin est correct pour votre joueur
@onready var spawn_sprite = $Cinematics/SpawnSprite  # Le sprite ou AnimationPlayer pour l'animation de spawn
@onready var point_light = $Lights/PointLight2D  # Assurez-vous que ce chemin est correct pour votre PointLight2D
@onready var door = $Interactable/Door  # Référence à la porte (mettre à jour si nécessaire)
